...
Upon the malware's execution it will automatically activates the bot and will now start waiting for responses in a private discord server where the attacker can send commands by using /command <SHELL COMMANDS HERE>
...
###### Added feature:
   ...
   *  Implemented binary to be a image file to contain needed object libraries 

###### Improvements tips:

   *  Obfuscate the token with xor and rot decoders and decode at runtime
   *  Shrink file in size (possibly by changing the api and using compressing method like bitwise compression)
